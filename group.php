...
// Check if user is already part of a group with 'accepted' status
$checkQuery = "SELECT * FROM group_members WHERE member_erpid = '$erpid' AND status = 'accepted'";
$checkResult = mysqli_query($conn, $checkQuery);
$hasGroup = mysqli_num_rows($checkResult) > 0;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$hasGroup) {
    ...
    $accepted = 'accepted';
    $stmt->bind_param("iss", $group_id, $erpid, $accepted);
    $stmt->execute();

    $pending = 'pending';
    foreach ([$member1, $member2, $member3] as $member) {
        if (!empty($member)) {
            $stmt->bind_param("iss", $group_id, $member, $pending);
            $stmt->execute();
        }
    }
    $stmt->close();
    ...
    foreach ([$member1, $member2, $member3] as $member) {
        if (!empty($member)) {
            $stmt->bind_param("issss", $group_id, $erpid, $member, $pending, $request_time);
            $stmt->execute();
        }
    }
    $stmt->close();
...
<body>
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" id="group_name" name="group_name" required <?= $hasGroup ? 'disabled' : '' ?>>
    </div>
    <div class="mb-3">
        <label for="project_title" class="form-label">Project Title</label>
        <input type="text" class="form-control" id="project_title" name="project_title" required <?= $hasGroup ? 'disabled' : '' ?>>
    </div>
    <div class="mb-3">
        <label for="project_field" class="form-label">Project Field</label>
        <input type="text" class="form-control" id="project_field" name="project_field" required <?= $hasGroup ? 'disabled' : '' ?>>
    </div>

    <?php for ($i = 1; $i <= 3; $i++): ?>
        <div class="mb-3">
            <label for="member<?= $i ?>" class="form-label">Select Member <?= $i ?></label>
            <select class="form-select" id="member<?= $i ?>" name="member<?= $i ?>" <?= $hasGroup ? 'disabled' : '' ?>>
    ...
    <?php if ($hasGroup): ?>
        <div class="alert alert-warning text-center">
            <strong>Note:</strong> You are already part of a group. You cannot create another group.
        </div>
    <?php endif; ?>

    <button type="submit" class="btn btn-primary w-100 mt-2" <?= $hasGroup ? 'disabled' : '' ?>>
